package com.ripple.blog.controller.admin;
...
import com.ripple.blog.domain.model.DTable;
import com.ripple.blog.domain.model.Post;
import com.ripple.blog.domain.model.Result;
import com.ripple.blog.domain.model.UploadResult;
import com.ripple.blog.domain.service.FileUploadService;
import com.ripple.blog.domain.service.PostService;
import com.ripple.blog.domain.service.TopicService;
import com.ripple.blog.infrastructure.common.annotation.DocDocument;
...
import com.ripple.blog.infrastructure.dao.model.FileType;
...
//        }
		...
		// topicEntitys有判空
		PostEntity entity = PostEntity.create(post.getId(), context.getId(), post.getTitle(), post.getContent(), topics,
				context.getEname());
	...
	private Result updatePost(HttpServletRequest request, @RequestBody Post post) {
		...
		Map<String, String> oldTopics = oldTopicNames.stream()
				.collect(Collectors.toMap(TopicEntity::getTopicName, TopicEntity::getId));
		...
		if (CollectionUtils.isNotEmpty(increment)) {
			incrementTopicEntity = topicService.findTopicByTopicNameAndIncCount(Joiner.on(",").join(increment),
					context);
		}
	...
	public Result enable(@PathVariable("ename") String ename, @PathVariable("shamId") String shamId,
	...
	public Result delete(@PathVariable("ename") String ename, @PathVariable("shamId") String shamId,
	...
	@ResponseBody
	public UploadResult upload(@RequestParam("upload") MultipartFile file, HttpServletRequest request,
		...
		try {
			...
			if (Objects.isNull(context)) {
				return UploadResult.create(fileName, url,
						BlogBusinessExceptionCode.USER_INFORMATION_ACQUISITION_FAILED.getMessage());
			}
			// 获取文件的后缀名
			...
			if (!Strings.isNullOrEmpty(md5)) {
				...
				if (Objects.nonNull(dataByMd5)) {
					...
					log.info("图片已上传 文件名称:{},别名:{},md5:{}", dataByMd5.getFileName(), dataByMd5.getNewName(),
							dataByMd5.getMd5());
				} else {
					FileUtils.copyInputStreamToFile(file.getInputStream(), path);
					FileUploadEntity fileUploadEntity = FileUploadEntity.create(uploadType, fileName, newFileName, md5,
							suffixName, cdnServer + uploadType + "/" + newFileName, context);
